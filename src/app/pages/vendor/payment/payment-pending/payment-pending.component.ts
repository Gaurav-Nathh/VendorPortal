...
exportToExcel(): void {
  ...
  const columnHeaders = ['Date', 'Type', 'Ref No.', 'Invoice', 'Payment', 'Adjustment', 'Balance', 'Due Date', 'Over Due'];
  ...
  // ===== 4. Total Row =====
  const totalInvoice = this.data.reduce((sum, item) => sum + (item.BILLAMT ?? 0), 0);
  const totalPayment = this.data.reduce((sum, item) => sum + (item.PAYAMT ?? 0), 0);
  const totalAdjustment = this.data.reduce((sum, item) => sum + (item.ADJAMT ?? 0), 0);
  const totalBalance = this.data.reduce((sum, item) => sum + (item.BALANCE ?? 0), 0);
  ...
  const footer3 = worksheet.addRow([`Generated on: ${new Date().toLocaleString()}`]);
